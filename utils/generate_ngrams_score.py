...
def calculate_ngram_scores(ngram_counts: dict) -> dict:
    ...
    for n_gram, count in ngram_counts.items():
        precalculated_values["counts"][normalize_solution(n_gram)] = (
            precalculated_values["counts"].get(normalize_solution(n_gram), 0) + count
        )
    ...
    for n_gram, count in precalculated_values["counts"].items():
        ...
        precalculated_values["min"] = (
            min(precalculated_values["min"], count)
            if precalculated_values["min"]
            else count
        )
    ...
    return {
        n_gram: round(
            max(
                1
                - (
                    (precalculated_values["max"] - count)
                    / max(precalculated_values["max"] - precalculated_values["min"], 1)
                ),
                0.05,
            ),
            2,
...
if __name__ == "__main__":
    ...
    result = {
        ...
        "solutions": calculate_solutions_ngram_scores(
            solutions_filepath=solutions_filepath
        ),
    }
